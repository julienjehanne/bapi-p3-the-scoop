...
function createComment (url, request){
  ...
  if (requestComment && requestComment.body && requestComment.username &&
     ...
     database.articles[requestComment.articleId]){
       // We create a comment object with the provided info in requestComment
        const comment ={
...
function updateComment(url, request){
  ...
  if(!database.comments[id]){
    ...
    database.comments[id].body = request.body.comment.body || database.comments[id].body;

    response.body = {comment: request.body.comment};
    response.status = 200;
  }
  return response;
}
...
function deleteComment(url, request){
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const saveComment = database.comments[id];
  const response = {};

  if(database.comments[id]){
    // Delete comment reference from article commentIds
    const articleCommentIds = database.articles[saveComment.articleId].commentIds;
    articleCommentIds.splice(articleCommentIds.indexOf(id),1);

    //Delete comment reference from user commentIds
    const userCommentIds = database.users[saveComment.username].commentIds;
    userCommentIds.splice(userCommentIds.indexOf(id),1);
...
function upvoteComment(url,request){
  const response = {};
  // Return 400 if request.body dosent exist
  ...
  if(id && database.users[username] && database.comments[id]){

    database.comments[id] = upvote(savedComment, username);
...
function downvoteComment(url,request){
  ...
  if(id && database.users[username] && database.comments[id]){
    ...
    database.comments[id] = downvote(savedComment, username);
...
function loadDatabase(){
  ...
  try {
    savedDatabase = yaml.safeLoad(fs.readFileSync('./savedDatabase.yaml'));
  } catch (e) {
    console.log(e);
  }
  if(typeof savedDatabase !== 'undefined'){
...
function saveDatabase(){
  ...
  const fs = require('fs');
    ...
    fs.writeFile('./savedDatabase.yaml', yaml.safeDump(database), (err) => {
      if (err) throw err;});
}
